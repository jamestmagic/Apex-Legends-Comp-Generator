...
const roster = [bloodhound, gibraltar, lifeline, pathfinder, wraith, bangalore, caustic, mirage,
  octane, wattson, crypt0, revenant, loba, rampart, horizon, fuse, valkyrie, seer, ash, madMaggie, newCastle,
  vantage];

const generateButton = document.getElementById('generateButton')
generateButton.addEventListener("click", function() {shuffle();})
...
function shuffle(type0, type1, type2){
  ...
  document.getElementById("type0").value = typeList0.options[typeList0.selectedIndex].text;
  var typeList1 = document.getElementById("typeList1");
  document.getElementById("type1").value = typeList1.options[typeList1.selectedIndex].text;
  var typeList2 = document.getElementById("typeList2");
  document.getElementById("type2").value = typeList2.options[typeList2.selectedIndex].text;

  if(document.getElementById("type2").value == "Support" && document.getElementById("type1").value == "Support" && document.getElementById("type0").value == "Support"){
    document.getElementById("type2").value = "Any";
    document.getElementById("supportError").innerHTML = "Cannot have 3 Support Legends";
  }
  else{
  ...
  for(let i = 0;i<roster.length;i++){
    ...
    if(roster[i].type == type1 && roster[i].name != randomLegends[0].name){
      randomLegends[1] = roster[i];
      console.log("#2 - " + i + ". " + roster[i].name + roster[i].value);
      break;
    }
  ...
  for(let i = 0;i<roster.length;i++){
    if(type2 == "Any" && roster[i].name != randomLegends[0].name && roster[i].name != randomLegends[1].name){
      randomLegends[2] = roster[i];
      console.log("#3 - " + i + ". " + roster[i].name + roster[i].value);
      break;
    }
    if(roster[i].type == type2 && roster[i] != randomLegends[0] && roster[i].name != randomLegends[1].name){
      randomLegends[2] = roster[i];
      console.log("#3 - " + i + ". " + roster[i].name + roster[i].value);
      break;
    }
  ...
  document.getElementById("legend0").src = "Legends/" + randomLegends[0].fileName;
  document.getElementById("legend1").src = "Legends/" + randomLegends[1].fileName;
  document.getElementById("legend2").src = "Legends/" + randomLegends[2].fileName;


  document.getElementById("legend0Type").src = "Types/" + randomLegends[0].type.toLowerCase() + ".png";
  document.getElementById("legend1Type").src = "Types/" + randomLegends[1].type.toLowerCase() + ".png";
  document.getElementById("legend2Type").src = "Types/" + randomLegends[2].type.toLowerCase() + ".png";

  document.getElementById("legend0Name").innerHTML = randomLegends[0].name.toUpperCase();
  document.getElementById("legend1Name").innerHTML = randomLegends[1].name.toUpperCase();
  document.getElementById("legend2Name").innerHTML = randomLegends[2].name.toUpperCase();
